...
// | MS56xx readPROM |
// +-----------------+
bool MS56xx::readPROM( MYBUS* usb ) throw( std::runtime_error ) {
  ...
  for( char i=0; i<8; i++ ) {
    ...
    int status = usb->writeBusUnreg( address, buff, 1 );
    if( status != 1 ) {
      throw ( std::runtime_error( "MS56xx device 'EPROM read' write error" ) );
      return false;
    } 
    status = usb->readBusUnreg( address, buff, 2 );
    if( status != 2 ) {
      throw ( std::runtime_error( "MS56xx device 'EPROM read' read error" ) );
      return false;
    }
  ...
  if( storCRC != evalCRC ) {
    throw ( std::runtime_error( "MS56xx " ) );
    return false;
  }
  return true;
}
...
// | MS56xx readADC |
// +----------------+
bool MS56xx::readADC( MYBUS* usb, char command, int& adc ) throw( std::runtime_error ) {
  ...
  int status = usb->writeBusUnreg( address, buff, 1 );
  if( status != 1 ) {
    throw ( std::runtime_error( "MS56xx device ADC read error" ) );
    return false;
  }
  ...
  status = usb->writeBusUnreg( address, buff, 1 );
  if( status != 1 ) {
    throw ( std::runtime_error( "MS56xx device ADC read error" ) );
    return false;
  }
  status = usb->readBusUnreg( address, buff, 3 );
  if( status != 3 ) {
    throw ( std::runtime_error( "MS56xx device ADC read error" ) );
    return false;
  }  
...
// | MS56xx writeCommand |
// +---------------------+
bool MS56xx::writeCommand( MYBUS* usb, char command, int ms ) throw( std::runtime_error ) {
  ...
  int status = usb->writeBusUnreg( address, buff, 1 );
  if( status != 1 ) {
    throw ( std::runtime_error( "MS56xx device write Command error" ) );
    return false;
  }
...
// | MS56xx init |
// +-------------+
bool MS56xx::init( MYBUS* usb ) throw( std::runtime_error ) {
  try {
    writeCommand( usb, MS56xx_Reset, 3 );
    readPROM( usb );
...
// +------------------------+
bool MS56xx::readRawPressure( MYBUS* usb, int& ipress ) throw( std::runtime_error ) {
  try {
    readADC( usb, MS56xx_ADC_D1 | MS56xx_OSR_4096, ipress );
  } catch( std::runtime_error &e ) {
    std::cout << e.what() << std::endl;
    return false;
  }
...
// +---------------------------+
bool MS56xx::readRawTemperature( MYBUS* usb, int& itemp ) throw( std::runtime_error ) {
  try {
    readADC( usb, MS56xx_ADC_D2 | MS56xx_OSR_4096, itemp );
  } catch( std::runtime_error &e ) {
    std::cout << e.what() << std::endl;
    return false;
  }
...
// | MS56xx getPressure |
// +--------------------+
bool MS56xx::getPressure( MYBUS* usb, double& press ) throw( std::runtime_error ) {

  try {
    int itemp = 0;
    readRawTemperature( usb, itemp );
...
// +-----------------------+
bool MS56xx::getTemperature( MYBUS* usb, double& temp ) throw( std::runtime_error ) {
...
// | MS56xx getAltitude |
// +--------------------+
bool MS56xx::getAltitude( MYBUS* usb, double& alt ) throw( std::runtime_error ) {

  try {
    double press = 0;
    getPressure( usb, press );
